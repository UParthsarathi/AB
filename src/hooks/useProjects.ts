...
export function useProjects() {
  ...
  const updateMilestoneStatus = useCallback((projectId: string, milestoneId: string, status: MilestoneStatus) => {
    commit(projects.map(p => {
      if (p.id === projectId) {
        const newMilestones = p.milestones.map(m => m.id === milestoneId ? { ...m, status } : m);
